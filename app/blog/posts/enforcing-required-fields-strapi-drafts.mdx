...
const getEmptyRequiredFields = (event, customComponents) => {
  ...
  return Object.entries(modelSchema.attributes)
    ...
    .reduce((acc, fieldName) => {
      ...
      // field in Strapi which has been set by the user
      if (typeof fieldValue === 'boolean') {
        return acc;
      }

      // Some fields types have string data, others have arrays
      // Media fields have an integer (id?) but `.length` on a number is undefined
      if (!fieldValue || (!fieldValue.length && typeof fieldValue !== 'number')) {
...
checks (for instance, `false` is a valid value for a boolean field, and a number
field will return `undefined` for `.length`, etc).
